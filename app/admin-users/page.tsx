    ...
    <div style={{ minHeight:'100vh', backgroundColor:'#f8fafc', fontFamily:'sans-serif' }}>
      ...
      <main style={{ padding:'40px', maxWidth:'800px', margin:'0 auto' }}>
        ...
        <div style={{ backgroundColor:'#fff', border:'1px solid #e2e8f0', borderRadius:'12px', padding:'32px', marginBottom:'32px', boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
          ...
          <p style={{ fontSize:'13px', color:'#64748b', marginBottom:'16px' }}>※ 事前にSupabaseでアカウントを作成してからメールアドレスを登録してください</p>
          ...
          <button onClick={handleAdd} disabled={loading} style={{ padding:'10px 28px', backgroundColor:'#1e3a5f', color:'#fff', border:'none', borderRadius:'8px', cursor:'pointer', fontSize:'15px', fontWeight:'bold' }}>{loading ? '追加中...' : '管理者に追加する'}</button>
        </div>
        {/* 管理者一覧 */}
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ backgroundColor:'#1e3a5f' }}>
                ...
                <th style={{ padding:'14px 20px', textAlign:'left', color:'#fff', fontSize:'13px' }}>登録日時</th>
                <th style={{ padding:'14px 20px', textAlign:'center', color:'#fff', fontSize:'13px' }}>操作</th>
              </tr>
            </thead>
            <tbody>
              {admins.length === 0 && (
                <tr><td colSpan={3} style={{ padding:'24px', textAlign:'center', color:'#94a3b8' }}>管理者がいません</td></tr>
              )}
              {admins.map((admin, i) => (
                <tr key={admin.id} style={{ backgroundColor: i % 2 === 0 ? '#fff' : '#f8fafc', borderTop:'1px solid #e2e8f0' }}>
                  ...
                  <td style={{ padding:'14px 20px', color:'#64748b', fontSize:'14px' }}>{new Date(admin.created_at).toLocaleDateString('ja-JP')}</td>
